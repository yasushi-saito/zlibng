...
ZLIB_INTERNAL block_state deflate_slow(deflate_state *s, int flush) {
    ...
    for (;;) {
        ...
        if (s->prev_length >= MIN_MATCH && s->match_length <= s->prev_length) {
            ...
            zng_zng_tr_tally_dist(s, s->strstart -1 - s->prev_match, s->prev_length - MIN_MATCH, bflush);
...
#else
            {
                ...
                if (max_insert > s->strstart) {
                    ...
                    zng_functable.insert_string(s, s->strstart + 1, insert_cnt);
                }
                s->prev_length = 0;
                s->match_available = 0;
                s->match_length = MIN_MATCH-1;
            ...
            zng_zng_tr_tally_lit(s, s->window[s->strstart-1], bflush);
            if (bflush) {
