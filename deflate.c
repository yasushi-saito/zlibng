...
const char zng_zng_deflate_copyright[] = " deflate 1.2.11.f Copyright 1995-2016 Jean-loup Gailly and Mark Adler ";
/*
    ...
    if (strm->avail_in != 0 || s->lookahead != 0 || (flush != Z_NO_FLUSH && s->status != FINISH_STATE)) {
        ...
        if (bstate == block_done) {
            if (flush == Z_PARTIAL_FLUSH) {
                ...
                zng_zng_tr_stored_block(s, (char*)0, 0L, 0);
...
static block_state deflate_stored(deflate_state *s, int flush) {
    ...
    do {
        ...
        zng_zng_tr_stored_block(s, (char *)0, 0L, last);
    ...
    if (left >= min_block ||
        ...
        last = flush == Z_FINISH && s->strm->avail_in == 0 &&
               len == left ? 1 : 0;
        zng_zng_tr_stored_block(s, (char *)s->window + s->block_start, len, last);
        s->block_start += len;
        flush_pending(s->strm);
    }
...
static block_state deflate_rle(deflate_state *s, int flush) {
    ...
    for (;;) {
        ...
        if (s->match_length >= MIN_MATCH) {
            ...
            zng_zng_tr_tally_dist(s, 1, s->match_length - MIN_MATCH, bflush);
