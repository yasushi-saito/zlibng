...
int ZEXPORT PREFIX(inflateResetKeep)(PREFIX3(stream) *strm) {
    ...
    state->mode = HEAD;
    state->check = zng_functable.adler32(0L, NULL, 0);
    state->last = 0;
    state->havedict = 0;
    state->dmax = 32768U;
    state->head = NULL;
...
#  define UPDATE(check, buf, len) \
    (state->flags ? PREFIX(crc32)(check, buf, len) : zng_functable.adler32(check, buf, len))
#else
#  define UPDATE(check, buf, len) zng_functable.adler32(check, buf, len)
#endif
...
int ZEXPORT PREFIX(inflate)(PREFIX3(stream) *strm, int flush) {
    ...
    for (;;)
        ...
        case HEAD:
            ...
            strm->adler = state->check = zng_functable.adler32(0L, NULL, 0);
...
int ZEXPORT PREFIX(inflateSetDictionary)(PREFIX3(stream) *strm, const unsigned char *dictionary, unsigned int dictLength) {
    ...
    if (state->mode == DICT) {
        dictid = zng_functable.adler32(0L, NULL, 0);
        dictid = zng_functable.adler32(dictid, dictionary, dictLength);
